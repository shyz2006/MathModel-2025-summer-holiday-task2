...
# 约束2: 销量不超过市场需求
model.addConstrs((Z[t, i, k] <= Request[i][k]
    for t in range(T) for i in range(I) for k in range(K)), name="Demand_Limit")

# 约束3: 是否种植该作物
model.addConstrs((X[t, i, j, k] <= M * Y[t, i, j, k]
    for t in range(T) for i in range(I) for j in range(J) for k in range(K)), name="X_UpperBound_Y")

model.addConstrs((X[t, i, j, k] >= 0.01 * Y[t, i, j, k]
    for t in range(T) for i in range(I) for j in range(J) for k in range(K)), name="X_LowerBound_Y")
